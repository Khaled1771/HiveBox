pipeline {
    ...
    stages {
        ...
        stage("Ansible & Kubernetes") {
            steps {
                script {
                    def hivebox_image_id = sh(script: "docker exec kind-control-plane crictl images | grep hivebox-img | awk '{print \$3}'", returnStdout: true)    // Show the image's ID to delete it
